/**
  *
  */
package cn.cqjtyy.mock


import java.util.UUID

import cn.cqjtyy.commons.model.{ProductInfo, UserInfo, UserVisitAction}
import cn.cqjtyy.commons.utils.{DateUtils, StringUtils}
...
object MockDataGenerate {
    ...
    for ( i <- 0 to 100) {
      ...
      for (j <- 0 to 10) {
        ...
        for (k <- 0 to random.nextInt(100)) {
          ...
          val cityid = random.nextInt(10).toString
          // 随机确定用户在当前session中的行为
          val action = actions(random.nextInt(4))
          // 根据随机产生的用户行action决定对应字段的值
          action match {
  ...
  private def mockUserInfo(): Array[UserInfo] ={
    ...
    for (i <- 0 to 100) {
      ...
      val professsional = "professional" + random.nextInt(100)
      val city = "city" + random.nextInt(100)
      val sex = sexes(random.nextInt(2))
      rows += UserInfo(userid, username, name, age, professsional, city, sex)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val usrInfoData=this.mockUserInfo()
    val productInfoData=this.mockProductInfo()
    //将模拟数据转换为RDD，使用makeRDD()方法
    val userVisitActionRdd = spark.sparkContext.makeRDD(userVisitActionData)
    val userInfoRdd = spark.sparkContext.makeRDD(usrInfoData)
