...
import cn.cqjtyy.commons.utils.ParamUtils
...
object AreaTop3Stat {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //tmp_area_baseic_info：表中的一条数据就代表一次点击商品的行为
    getAreaPidBaseInfoTable(sparkSession,cityId2PidRDD,cityId2AreaInfoRDD)
    ...
    //sparkSession.sql("select * from tmp_area_baseic_info").show()
  ...
  def getTop3Product(sparkSession: SparkSession, taskUUID: String): Unit = {
    /*val sql = "select area,city_infos,pid,product_name,produc_status,click_count," +
    ...
    "city_infos,pid,product_name,product_status,click_count from (" +
      "select area,city_infos,pid,product_name,produc_status,click_count," +
      "row_number() over(PARTITION BY area ORDER BY click_count DESC) rank from tmp_area_count_product_info)" +
    " t where rank<=3"
  ...
  def getAreaProductClickCountInfo(sparkSession: SparkSession): Unit = {
    //tmp_area_count_product_info:area,city_infos,pid,click_count tacc
  ...
  def getAreaProductClickCountTable(sparkSession: SparkSession): Unit = {
    val sql ="select area,pid,count(*) click_count," +
      "group_concat_distinct(concat_long_string(city_id,city_name,':')) city_ infos " +
      "from tmp_area_baseic_info group by area,pid"
    sparkSession.sql(sql).createOrReplaceTempView("tmp_area_click_count")
  }

  def getAreaPidBaseInfoTable(sparkSession: SparkSession,
  ...
  def getCityAndProductInfo(sparkSession: SparkSession, taskParam: JSONObject) = {
    ...
    val sql = "select city_id,click_product_id form user_visit_action where date>='" + startDate +
