...
import cn.cqjtyy.commons.model.UserVisitAction
import cn.cqjtyy.commons.utils.{DateUtils, ParamUtils}
...
object PageConvertStat {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //sessionId2ActionRDD:RDD[(sessioinId,action)]
    ...
    val pageSpllitNumRDD = sessionId2GroupRDD.flatMap{
      case (sessionId,iterableAction) =>
        ...
        //pageList.slice(0,pageList.length - 1):[1,2,3,4,...,N-1]
    ...
    val startPageCount = sessionId2ActionRDD.filter{
      case (sessionId,action) => action.page_id == startPage.toLong
    }.count()
  ...
  def getPageConvert(sparkSession: SparkSession,
    ...
    for(pageSplit <- targetPageSplit){
      //第一次循环 1_2 count
      val currentPageCount = pageSplitCountMap.get(pageSplit).get.toDouble
